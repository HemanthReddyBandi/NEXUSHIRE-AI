...
export default function App() {
  ...
  const [hrIdFile, setHrIdFile] = useState(null);
  ...
  return (
    <p style={{ marginTop: 12 }}>
      ⏱ Waiting Time: <strong>{seconds}s</strong>
    </p>
  );
}
  ...
  function onStopAnswer(answerText) {
    ...
    if (currentIndex === -1) {
      const resumeQs = [
        ...
        "What would you improve in your skills?",
        "What was your role during the internship?",
        "How did you design the solution of your major project?",
        "Did you build this project alone or in a team?",
        "If your teammate is removed, can you rebuild it?"
      ];
...
Emotion: ${a.emotion}`,
  ...
  {/* BACK BUTTON */}
  {mode !== "home" && (
    <button onClick={() => setMode("home")} style={{ marginBottom: 15 }}>
      ⬅ Back
    </button>
  )}
  ...
  {mode === "home" && (
    <div className="card">
      ...
      <button onClick={() => setMode("hr_live_waiting")}>
        🎓 HR Live Interview
      </button>
    </div>
  )}
...
{mode === "hr_room" && (
  <div className="card">
    ...
    <input
      ...
      onChange={(e) => setHrIdFile(e.target.files[0])}
      style={{ marginBottom: "10px" }}
    />

    {hrIdFile && (
      <p style={{ color: "#22c55e", fontSize: "0.85rem" }}>
        ✔ {hrIdFile.name} uploaded
      </p>
    )}

    <button
      disabled={!hrIdFile}
      onClick={() => alert("HR Room created (demo mode)")}
      style={{
        width: "100%",
        marginTop: "12px",
        opacity: hrIdFile ? 1 : 0.5
      }}
    >
      Enter HR Room
    </button>
  ...
  {/* ---------------- HR LIVE WAITING ---------------- */}
  {mode === "hr_live_waiting" && (
    <div className="card" style={{ textAlign: "center" }}>
      <h3>⏳ Waiting in Queue</h3>

      <p>No HR available at the moment</p>
